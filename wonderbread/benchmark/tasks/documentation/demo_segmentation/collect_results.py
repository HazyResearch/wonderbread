...
if __name__ == "__main__":
    ...
    for demo_folder in os.listdir(path_to_input_dir):
        ...
        for file in os.listdir(os.path.join(path_to_input_dir, demo_folder)):
            if file.startswith('task_segmentation__') and file.endswith('.csv'):
    ...
    if len(results) > 0:
        ...
        df.to_csv(os.path.join(path_to_output_dir, 'task_segmentation_all_results.csv'), index=False)
        print(f"Saved output CSV to {os.path.join(path_to_output_dir, 'task_segmentation_all_results.csv')}")